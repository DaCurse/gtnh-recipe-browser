...
class WordReader {
  ...
  readonly stringCache = new Map<number, string>();

  constructor(compressed: Uint8Array) {
    try {
      this.bytes = gunzipSync(compressed);
    } catch (error) {
      throw new PackDecodeError(`Unable to decompress data.bin: ${error instanceof Error ? error.message : String(error)}`);
    }
    if (this.bytes.byteLength % 4 !== 0) {
      throw new PackDecodeError(`Decoded data length ${this.bytes.byteLength} is not aligned to 32-bit words`);
    }
    this.words = new Int32Array(this.bytes.buffer, this.bytes.byteOffset, this.bytes.byteLength / 4);
    if (this.words.length < ROOT_WORDS) {
      throw new PackDecodeError(`Decoded repository is truncated: expected at least ${ROOT_WORDS} words`);
    }
  }

  int(offset: number, context: string): number {
    if (!Number.isInteger(offset) || offset < 0 || offset >= this.words.length) {
      throw new PackDecodeError(`${context}: word offset ${offset} is outside 0..${this.words.length - 1}`);
    }
  ...
  slice(pointer: number, context: string): Int32Array {
    ...
    if (end > this.words.length) {
      throw new PackDecodeError(`${context}: slice ending at ${end} exceeds ${this.words.length} words`);
    }
  ...
  string(pointer: number | null, context: string): string | null {
    ...
    if (byteLength < 0) throw new PackDecodeError(`${context}: negative string length ${byteLength}`);
    const begin = (pointer + 1) * 4;
    const end = begin + byteLength;
    if (end > this.bytes.byteLength) {
      throw new PackDecodeError(`${context}: string ending at byte ${end} exceeds ${this.bytes.byteLength}`);
    }
    let decoded: string;
    try {
      decoded = textDecoder.decode(this.bytes.subarray(begin, end));
    } catch (error) {
      throw new PackDecodeError(`${context}: invalid UTF-8 (${error instanceof Error ? error.message : String(error)})`);
    }
