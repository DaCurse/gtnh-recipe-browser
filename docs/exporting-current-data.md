...
compatibility, disables BugTorch, installs the production exporter jars, and writes
...
builds the pack twice, compares every generated byte, and verifies hashes plus sampled sprites. Publishing copies
the immutable pack into `public/data/` and prepends it to `public/versions.json`, retaining 2.8 for switching.

The verified 2.9.0-beta-2 run produced dataset `2.9.0-beta-2-r0ba2f2c5584c`. Its provenance is stored beside the
manifest under `public/data/2.9.0-beta-2-r0ba2f2c5584c/`.
